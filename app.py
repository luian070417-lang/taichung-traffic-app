...
# ==========================================
# ⚡ 新增功能：雙向互動！接收長官 LINE 查詢 (Webhook 接收端)
# ==========================================
# 我們利用 Streamlit 的 query_params 做出一個祕密通道，讓 LINE 把訊息丟進來
query_params = st.query_params
if "webhook" in query_params:
    st.write("LINE Webhook 運作中...")
    try:
        # 解析 LINE 傳過來的長官訊息
        req_body = st.context.request.body.read().decode("utf-8")
        req_json = json.loads(req_body)
        events = req_json.get("events", [])
        
        if events:
            event = events[0]
            user_msg = event.get("message", {}).get("text", "").strip() # 長官傳的字（例如：西屯區）
            sender_id = event.get("source", {}).get("userId", LINE_USER_ID)
            
            # 如果長官傳的名字在臺中市的行政區清單裡
            if user_msg in df['區'].unique():
                sub_df = df[df['區'] == user_msg]
                count = len(sub_df)
                top_cause = sub_df['肇事因素主要'].value_counts().idxmax()
                
                # 交通部標準代碼中文對照
                cause_map = {7.0: "未依規定讓車", 59.0: "闖紅燈", 84.0: "未注意車前狀況"}
                cause_zh = cause_map.get(top_cause, f"未知代碼 {top_cause}")
                
                reply_text = f"📊【臺中交通戰情室回報】\n長官您好，已為您即時調閱【{user_msg}】歷史大數據：\n\n累積總事故量：{count} 件\n頭號肇事病因：代碼 {top_cause} ({cause_zh})\n\n系統建議：該區高風險交叉路口應優先加強工程稽核或科技執法！"
            else:
                reply_text = f"🤖【系統提示】\n長官您好，查無「{user_msg}」行政區。請嘗試輸入：西屯區、北屯區、后里區 等臺中市行政區名稱。"
            
            # 立刻把查到的資料傳回長官手機
            send_line_official_alert(reply_text, to_user=sender_id)
    except:
        pass
    st.stop() # Webhook 模式下不渲染網頁主視覺
...
with tab2:
    ...
    st.write("本後台由 AI Agent 自動追蹤工務局、建設局的實際施工項目，並與 AI 建議進行對照稽核.0")

...
# ==========================================
# ⚡ 雙向互動祕密通道 (更安全的 Webhook 處理)
# ==========================================
# 獲取目前的網址參數
query_params = st.query_params

# 當 LINE 伺服器傳訊息過來時，只會觸發這裡，絕對不影響主網頁運作
if "webhook" in query_params or "code" in query_params:
    try:
        # 讀取 LINE 的事件
        body = st.context.request.body.read().decode("utf-8")
        ...
        if events:
            ...
            if user_text in df['區'].unique():
                sub_df = df[df['區'] == user_text]
                count = len(sub_df)
                top_cause = sub_df['肇事因素主要'].value_counts().idxmax()
                
                # 代碼中文轉換
                cause_map = {7.0: "未依規定讓車", 59.0: "闖紅燈", 84.0: "未注意車前狀況"}
                cause_zh = cause_map.get(top_cause, f"未知原因(代碼 {top_cause})")
                
                reply_text = f"📊【臺中交通戰情室回報】\n長官您好，已即時調閱【{user_text}】歷史特徵：\n\n累積事故總量：{count} 件\n主要肇事因素：{cause_zh}\n\n系統建議：該區相關路口應優先列入限額改善工程排程。"
            else:
                reply_text = f"🤖【系統提示】\n長官您好，查無「{user_text}」行政區。請輸入：西屯區、北屯區、后里區 等臺中市行政區名稱進行查詢。"
            
            # 使用官方最穩定的 push 機制回傳給長官
